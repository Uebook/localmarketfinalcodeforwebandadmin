...
export default function ProductList() {
  ...
  const loadProducts = async () => {
    try {
      ...
      // Debug: Log products with category_ids
      if (productsList.length > 0) {
        const withCategories = productsList.filter(p => p.category_id);
        console.log(`Loaded ${productsList.length} products, ${withCategories.length} have category_id`);
        if (withCategories.length > 0) {
          console.log('Sample product category_ids:', withCategories.slice(0, 3).map(p => ({ name: p.name, category_id: p.category_id })));
        }
      }
                        ...
                        <div className="relative w-16 h-16">
                          {product.image_url ? (
                            <img
                              ...
                              onClick={() => setViewingImage({ url: product.image_url, name: product.name })}
                              onError={(e) => {
                          ...
                          <div 
                            className={`image-placeholder w-16 h-16 bg-gray-100 rounded-lg border border-gray-200 flex items-center justify-center ${product.image_url ? 'hidden' : 'flex'}`}
                          >
